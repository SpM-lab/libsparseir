#!/usr/bin/env python3
"""
Auto-generate fit_tau_impl.inc file for Fortran module.

This script generates fit_tau functions for 1d to 7d arrays
...
def generate_fit_tau_function(ndim, input_type, output_type):
    """Generate fit_tau function for given dimension and type combination."""
    ...
    # Determine if CMPLX conversion is needed
    if output_type == 'z':
        res_assignment = "res = CMPLX(res_c, KIND=DP)"
    else:
  ...
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('fit_tau_{func_suffix}_{ndim}d', &
...
def generate_all_functions():
    """Generate all fit_tau functions for 1d to 7d with different type combinations."""
    content = []
    
    # Header comment
    content.append("! Auto-generated file - do not edit manually")
    content.append("! Generated by generate_fit_tau.py")
    ...
    for input_type, output_type in input_output_types:
        for ndim in range(1, 8):  # 1d to 7d
            function = generate_fit_tau_function(ndim, input_type, output_type)
