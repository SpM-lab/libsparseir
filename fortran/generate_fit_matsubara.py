#!/usr/bin/env python3
"""
Auto-generate fit_matsubara_impl.inc file for Fortran module.

This script generates fit_matsubara functions for 1d to 7d arrays
...
def generate_fit_matsubara_function(ndim, input_type, output_type):
    """Generate fit_matsubara function for given dimension and type combination."""
    ...
    # C function
    c_function = f"c_spir_sampling_fit_{func_suffix}"
    
    # Generate the function
    function = f"""SUBROUTINE fit_matsubara_{func_suffix}_{ndim}d(obj, statistics, target_dim, arr, res)
  ...
  IF (target_dim < 1 .or. target_dim > {ndim}) THEN
    CALL errore('fit_matsubara_{func_suffix}_{ndim}d', 'Target dimension is out of range', 1)
  ENDIF
  SELECT CASE (statistics)
  CASE (SPIR_STATISTICS_FERMIONIC)
    IF (input_dims_c(target_dim) /= obj%nfreq_f) THEN
      CALL errore('fit_matsubara_{func_suffix}_{ndim}d', 'Target dimension is not the same as the number of Matsubara frequencies', 1)
    ENDIF
  CASE (SPIR_STATISTICS_BOSONIC)
    ...
    ENDIF
  CASE DEFAULT
    CALL errore('fit_matsubara_{func_suffix}_{ndim}d', 'Invalid statistics', 1)
    RETURN
  END SELECT
  IF (output_dims_c(target_dim) /= obj%size) THEN
    CALL errore('fit_matsubara_{func_suffix}_{ndim}d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('fit_matsubara_{func_suffix}_{ndim}d', &
...
def generate_all_functions():
    """Generate all fit_matsubara functions for 1d to 7d with different type combinations."""
    content = []
    
    # Header comment
    content.append("! Auto-generated file - do not edit manually")
    content.append("! Generated by generate_fit_matsubara.py")
    content.append("! Based on template from tmp.f90")
    content.append("")
    
    # Type combinations: (input_type, output_type)
    input_output_types = [('z', 'z'), ('z', 'd')]
    ...
    for input_type, output_type in input_output_types:
        for ndim in range(1, 8):  # 1d to 7d
            function = generate_fit_matsubara_function(ndim, input_type, output_type)
...
def main():
    ...
    output_file = "fit_matsubara_impl.inc"
    ...
    print(f"Type combinations: (z,z) and (z,d)")
