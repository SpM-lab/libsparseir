...
SUBROUTINE evaluate_matsubara_zz_1d(obj, statistics, target_dim, arr, res)
  ...
  CASE (SPIR_STATISTICS_FERMIONIC)
    IF (output_dims_c(target_dim) /= obj%nfreq_f) THEN
      CALL errore('evaluate_tau_zz_1d', 'Target dimension is not the same as the number of Matsubara frequencies', 1)
    ENDIF
  ...
  CASE DEFAULT
    CALL errore('evaluate_tau_zz_1d', 'Invalid statistics', 1)
    RETURN
  END SELECT
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zz_1d', 'Error evaluating on Matsubara frequencies', status_c)
  ENDIF
...
# 53 "fortran/fit_tau_impl.fh"
  IF (obj%positive_only) THEN
    ...
    status_c = c_spir_sampling_fit_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
    IF (status_c /= 0) THEN
      CALL errore('evaluate_tau_zz_1d', 'Error fitting on tau sampling points', status_c)
    ENDIF
    res = CMPLX(res_c, 0.0_DP, KIND=DP)
    DEALLOCATE(arr_c, res_c)
  ELSE
...
SUBROUTINE fit_matsubara_zz_1d(obj, statistics, target_dim, arr, res)
  ...
  END SELECT
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zz_1d', 'Error fitting on Matsubara frequencies', status_c)
  ENDIF
...
# 17 "fortran/ir2dlr_impl.fh"
  ...
  IF (SIZE(arr, target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zz_1d', 'Input dimension is not the same as the size of the IR', 1)
  ENDIF
  IF (SIZE(res, target_dim) /= obj%npoles) THEN
     CALL errore('evaluate_tau_zz_1d', 'Output dimension is not the same as the number of poles', 1)
  ENDIF
  target_dim_c = target_dim - 1
# 53 "fortran/ir2dlr_impl.fh"
  IF (obj%positive_only) THEN
    ...
    status_c = c_spir_ir2dlr_dd(obj%dlr_f_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
    IF (status_c /= 0) THEN
      CALL errore('evaluate_tau_zz_1d', 'Error converting IR to DLR', status_c)
    ENDIF
    res = CMPLX(res_c, 0.0_DP, KIND=DP)
    DEALLOCATE(arr_c, res_c)
  ELSE
...
# 17 "fortran/dlr2ir_impl.fh"
  ...
  IF (SIZE(arr, target_dim) /= obj%npoles) THEN
    CALL errore('evaluate_tau_zz_1d', 'Input dimension is not the same as the number of poles', 1)
  ENDIF
  IF (SIZE(res, target_dim) /= obj%size) THEN
     CALL errore('evaluate_tau_zz_1d', 'Output dimension is not the same as the size of the IR', 1)
  ENDIF
  target_dim_c = target_dim - 1
# 53 "fortran/dlr2ir_impl.fh"
  IF (obj%positive_only) THEN
    ...
    status_c = c_spir_dlr2ir_dd(obj%dlr_f_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
    IF (status_c /= 0) THEN
      CALL errore('evaluate_tau_zz_1d', 'Error converting DLR to IR', status_c)
    ENDIF
    res = CMPLX(res_c, 0.0_DP, KIND=DP)
    DEALLOCATE(arr_c, res_c)
  ELSE
