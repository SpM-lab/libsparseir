...
spir_sve_result* sve_logistic = spir_sve_result_new(logistic_kernel, epsilon, lmax, n_gauss, Twork, &status);
assert(status == SPIR_COMPUTATION_SUCCESS);
...
spir_sve_result* sve_regularized = spir_sve_result_new(regularized_kernel, epsilon, lmax, n_gauss, Twork, &status);
assert(status == SPIR_COMPUTATION_SUCCESS);
