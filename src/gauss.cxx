...
public:
    ...
    Rule<T> reseat(T a, T b) const {
        ...
        vector<T> new_x(x.size()), new_w(w.size()), new_x_forward(x_forward.size()), new_x_backward(x_backward.size());
    ...
    Rule<T> astype(const string& dtype) const {
        // Assuming dtype is either "float" or "double"
        return *this;
    }

    static Rule<T> join(const vector<Rule<T>>& gauss_list) {
        if (gauss_list.empty()) {
            return Rule<T>({}, {});
        }
        ...
        for (const auto& curr : gauss_list) {
            if (curr.a != prev_b) {
                throw invalid_argument("Gauss rules must be ascending");
            }
            prev_b = curr.b;
            vector<T> curr_x_forward(curr.x_forward.size()), curr_x_backward(curr.x_backward.size());
...
public:
    ...
    NestedRule(const vector<T>& x, const vector<T>& w, const vector<T>& v, const vector<T>& x_forward = {}, const vector<T>& x_backward = {}, T a = -1, T b = 1)
