...
TEST_CASE("FiniteTempBasis consistency tests", "[basis]") {
    ...
    SECTION("Singular value scaling")
    {
        ...
        sparseir::FiniteTempBasis<sparseir::Fermionic> basis(beta, omega_max, epsilon, std::static_pointer_cast<sparseir::AbstractKernel>(kernel), sve_result);
        ...
        REQUIRE(std::abs(basis.accuracy - sve_result.s(sve_result.s.size() - 1) / sve_result.s(0)) < 1e-10);
    }
    ...
    SECTION("default_sampling_points") {
        ...
        auto kernel = std::make_shared<sparseir::LogisticKernel>(beta * omega_max);
        auto sve_result = sparseir::compute_sve(*kernel, epsilon);
        auto basis = sparseir::FiniteTempBasis<sparseir::Fermionic>(beta, omega_max, epsilon, std::static_pointer_cast<sparseir::AbstractKernel>(kernel), sve_result);
        auto s = sve_result.s;
        //REQUIRE(s.size() == 32);
    ...
    SECTION("RegularizedBoseKernel epsilon = 1e-6")
    {
        ...
        auto basis = sparseir::FiniteTempBasis<sparseir::Bosonic>(beta, omega_max, rbk_ptr, sve_result);
        ...
        Eigen::VectorXd s_ref_eigen = Eigen::Map<Eigen::VectorXd>(s_ref.data(), s_ref.size());
    ...
    SECTION("RegularizedBoseKernel epsilon = 1e-10")
    {
        ...
        auto basis = sparseir::FiniteTempBasis<sparseir::Bosonic>(
            beta, omega_max, rbk_ptr, sve_result);
        Eigen::VectorXd s = basis.s;
        std::vector<double> s_ref = {
            0.10510068935158154, 0.09586154039284558, 0.030202857804989457,
            0.012403239133253087, 0.0036202319256581714, 0.0008751907696266496,
            0.00017856035016188457, 3.146583475667272e-5, 4.873570749208084e-6,
            6.72825223304962e-7, 8.374707823163109e-8, 9.48760053912297e-9,
