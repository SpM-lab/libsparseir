...
TEST_CASE("DiscreteExtrema")
{
    ...
    REQUIRE(sparseir::discrete_extrema(identity, nonnegative) ==
            std::vector<int>({8}));
    // REQUIRE(sparseir::discrete_extrema(shifted_identity, nonnegative) ==
    // std::vector<int>({0, 8})); REQUIRE(discrete_extrema(square, symmetric) ==
    // std::vector<int>({-8, 0, 8})); REQUIRE(discrete_extrema(constant,
    // symmetric) == std::vector<int>({}));
}
