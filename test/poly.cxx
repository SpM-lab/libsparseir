...
TEST_CASE("sparseir::PiecewiseLegendrePolyVector", "[poly]")
{
    ...
    // Create sparseir::PiecewiseLegendrePoly objects and wrap them in shared_ptr
    auto pwlp1 = std::make_shared<sparseir::PiecewiseLegendrePoly>(data1, knots1, l1);
    auto pwlp2 = std::make_shared<sparseir::PiecewiseLegendrePoly>(data2, knots2, l2);
    auto pwlp3 = std::make_shared<sparseir::PiecewiseLegendrePoly>(data3, knots3, l3);

    // Create sparseir::PiecewiseLegendrePolyVector
    std::vector<std::shared_ptr<sparseir::PiecewiseLegendrePoly>> polyvec = {pwlp1, pwlp2, pwlp3};
    sparseir::PiecewiseLegendrePolyVector polys(polyvec);

    // Test length
    REQUIRE(polys.size() == 3);

    // Test properties - note the -> operator for shared_ptr access
    ...
    Eigen::VectorXd xs(4);
    xs << -0.8, -0.2, 0.2, 0.8;
    Eigen::MatrixXd polys_xs = polys(xs);
...
TEST_CASE("func_for_part tests", "[poly]")
{
    ...
    REQUIRE(func_for_part(0) == uhat_full->operator()(0).real());
    REQUIRE(func_for_part(0) == Approx(0.9697214762855008));
    REQUIRE(func_for_part(1) == uhat_full->operator()(2).real());
    REQUIRE(func_for_part(1) == Approx(0.1581728503925827));
    REQUIRE(func_for_part(2) == uhat_full->operator()(4).real());
    REQUIRE(func_for_part(2) == Approx(0.05823707470687053));
    REQUIRE(func_for_part(3) == uhat_full->operator()(6).real());
