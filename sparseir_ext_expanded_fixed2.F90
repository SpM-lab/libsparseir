...
# 22 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 1
  IF (target_dim < 1 .or. target_dim > 1) THEN
    CALL errore('evaluate_tau_dz_1d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_dz_1d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_dz_1d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_dz_1d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_dz_1d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = CMPLX(res_c, 0.0_DP, KIND=DP)
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 22 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 2
  IF (target_dim < 1 .or. target_dim > 2) THEN
    CALL errore('evaluate_tau_dz_2d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_dz_2d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_dz_2d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_dz_2d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_dz_2d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(CMPLX(res_c, 0.0_DP, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 22 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 3
  IF (target_dim < 1 .or. target_dim > 3) THEN
    CALL errore('evaluate_tau_dz_3d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_dz_3d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_dz_3d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_dz_3d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_dz_3d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(CMPLX(res_c, 0.0_DP, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 22 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 4
  IF (target_dim < 1 .or. target_dim > 4) THEN
    CALL errore('evaluate_tau_dz_4d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_dz_4d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_dz_4d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_dz_4d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_dz_4d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(CMPLX(res_c, 0.0_DP, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 22 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 5
  IF (target_dim < 1 .or. target_dim > 5) THEN
    CALL errore('evaluate_tau_dz_5d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_dz_5d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_dz_5d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_dz_5d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_dz_5d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(CMPLX(res_c, 0.0_DP, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 22 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 6
  IF (target_dim < 1 .or. target_dim > 6) THEN
    CALL errore('evaluate_tau_dz_6d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_dz_6d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_dz_6d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_dz_6d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_dz_6d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(CMPLX(res_c, 0.0_DP, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 22 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 7
  IF (target_dim < 1 .or. target_dim > 7) THEN
    CALL errore('evaluate_tau_dz_7d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_dz_7d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_dz_7d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_dz_7d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_dz_7d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(CMPLX(res_c, 0.0_DP, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 13 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 1
  IF (target_dim < 1 .or. target_dim > 1) THEN
    CALL errore('evaluate_tau_zd_1d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_1d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_zd_1d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_zd_1d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_1d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = REAL(res_c, KIND=DP)
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 13 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 2
  IF (target_dim < 1 .or. target_dim > 2) THEN
    CALL errore('evaluate_tau_zd_2d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_2d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_zd_2d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_zd_2d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_2d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(REAL(res_c, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 13 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 3
  IF (target_dim < 1 .or. target_dim > 3) THEN
    CALL errore('evaluate_tau_zd_3d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_3d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_zd_3d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_zd_3d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_3d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(REAL(res_c, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 13 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 4
  IF (target_dim < 1 .or. target_dim > 4) THEN
    CALL errore('evaluate_tau_zd_4d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_4d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_zd_4d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_zd_4d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_4d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(REAL(res_c, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 13 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 5
  IF (target_dim < 1 .or. target_dim > 5) THEN
    CALL errore('evaluate_tau_zd_5d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_5d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_zd_5d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_zd_5d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_5d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(REAL(res_c, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 13 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 6
  IF (target_dim < 1 .or. target_dim > 6) THEN
    CALL errore('evaluate_tau_zd_6d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_6d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_zd_6d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_zd_6d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_6d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(REAL(res_c, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
# 13 "fortran/evaluate_tau_impl.fh"
  ...
  ndim_c = 7
  IF (target_dim < 1 .or. target_dim > 7) THEN
    CALL errore('evaluate_tau_zd_7d', 'Target dimension is out of range', 1)
  ENDIF
  IF (input_dims_c(target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_7d', 'Target dimension is not the same as the basis size', 1)
  ENDIF
  IF (output_dims_c(target_dim) /= obj%ntau) THEN
    CALL errore('evaluate_tau_zd_7d', 'Target dimension is not the same as the number of tau sampling points', 1)
  ENDIF
  IF (.NOT. check_output_dims(target_dim, input_dims_c, output_dims_c)) THEN
    CALL errore('evaluate_tau_zd_7d', &
        'Output dimensions are not the same as the input dimensions except for the TARGET dimension', 1)
  ENDIF
  target_dim_c = target_dim - 1
  ...
  status_c = c_spir_sampling_eval_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
    ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_7d', 'Error evaluating on tau sampling points', status_c)
  ENDIF
  res = RESHAPE(REAL(res_c, KIND=DP), shape(res))
  DEALLOCATE(arr_c, res_c)
# 77 "fortran/evaluate_tau_impl.fh"
...
SUBROUTINE evaluate_matsubara_zz_1d(obj, statistics, target_dim, arr, res)
  ...
  CASE (SPIR_STATISTICS_FERMIONIC)
    IF (output_dims_c(target_dim) /= obj%nfreq_f) THEN
      CALL errore('evaluate_tau_zd_7d', 'Target dimension is not the same as the number of Matsubara frequencies', 1)
    ENDIF
  ...
  CASE DEFAULT
    CALL errore('evaluate_tau_zd_7d', 'Invalid statistics', 1)
    RETURN
  END SELECT
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_7d', 'Error evaluating on Matsubara frequencies', status_c)
  ENDIF
...
# 53 "fortran/fit_tau_impl.fh"
  IF (obj%positive_only) THEN
    ...
    status_c = c_spir_sampling_fit_dd(obj%tau_smpl_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
    IF (status_c /= 0) THEN
      CALL errore('evaluate_tau_zd_7d', 'Error fitting on tau sampling points', status_c)
    ENDIF
    res = CMPLX(res_c, 0.0_DP, KIND=DP)
    DEALLOCATE(arr_c, res_c)
  ELSE
...
SUBROUTINE fit_matsubara_zz_1d(obj, statistics, target_dim, arr, res)
  ...
  END SELECT
  IF (status_c /= 0) THEN
    CALL errore('evaluate_tau_zd_7d', 'Error fitting on Matsubara frequencies', status_c)
  ENDIF
...
# 17 "fortran/ir2dlr_impl.fh"
  ...
  IF (SIZE(arr, target_dim) /= obj%size) THEN
    CALL errore('evaluate_tau_zd_7d', 'Input dimension is not the same as the size of the IR', 1)
  ENDIF
  IF (SIZE(res, target_dim) /= obj%npoles) THEN
     CALL errore('evaluate_tau_zd_7d', 'Output dimension is not the same as the number of poles', 1)
  ENDIF
  target_dim_c = target_dim - 1
# 53 "fortran/ir2dlr_impl.fh"
  IF (obj%positive_only) THEN
    ...
    status_c = c_spir_ir2dlr_dd(obj%dlr_f_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
    IF (status_c /= 0) THEN
      CALL errore('evaluate_tau_zd_7d', 'Error converting IR to DLR', status_c)
    ENDIF
    res = CMPLX(res_c, 0.0_DP, KIND=DP)
    DEALLOCATE(arr_c, res_c)
  ELSE
...
# 17 "fortran/dlr2ir_impl.fh"
  ...
  IF (SIZE(arr, target_dim) /= obj%npoles) THEN
    CALL errore('evaluate_tau_zd_7d', 'Input dimension is not the same as the number of poles', 1)
  ENDIF
  IF (SIZE(res, target_dim) /= obj%size) THEN
     CALL errore('evaluate_tau_zd_7d', 'Output dimension is not the same as the size of the IR', 1)
  ENDIF
  target_dim_c = target_dim - 1
# 53 "fortran/dlr2ir_impl.fh"
  IF (obj%positive_only) THEN
    ...
    status_c = c_spir_dlr2ir_dd(obj%dlr_f_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr_c), c_loc(res_c))
    IF (status_c /= 0) THEN
      CALL errore('evaluate_tau_zd_7d', 'Error converting DLR to IR', status_c)
    ENDIF
    res = CMPLX(res_c, 0.0_DP, KIND=DP)
    DEALLOCATE(arr_c, res_c)
  ELSE
