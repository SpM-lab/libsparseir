...
_gemm(const U &A, const V &B)
{
    return A * B;
}

template <typename U, typename V>
Eigen::Matrix<decltype(std::declval<typename U::Scalar>() * std::declval<typename V::Scalar>()), Eigen::Dynamic, Eigen::Dynamic>
_gemm2(const U &A, const V &B)
{
    return A * B;
}
          ...
          const Eigen::array<Eigen::IndexPair<int>, 1> &contract_dims)
{
    ...
    Eigen::Matrix<ResultType, Eigen::Dynamic, Eigen::Dynamic> result_matrix = _gemm2(matrix1, matrix2);
                              ...
                              const Eigen::MatrixXcd &B, bool has_zero)
{
    ...
    Eigen::MatrixXcd UHB = _gemm2(U_realT, B.real());
    UHB += _gemm2(U_imagT, B.imag());
