          ...
          const Eigen::array<Eigen::IndexPair<int>, 1> &contract_dims)
{
    ...
    // Move contract dimensions to the last position for tensor1 and first
    // position for tensor2 Convert to Eigen::Tensor first, then apply movedim
    Eigen::Tensor<ResultType, N1> tensor1_tensor =
        tensor1.template cast<ResultType>();
    Eigen::Tensor<ResultType, N2> tensor2_tensor =
        tensor2.template cast<ResultType>();
    ...
    Eigen::Map<const Eigen::Matrix<ResultType, Eigen::Dynamic, Eigen::Dynamic>>
        matrix1(tensor1_moved.data(), rows1, cols1);
    Eigen::Map<const Eigen::Matrix<ResultType, Eigen::Dynamic, Eigen::Dynamic>>
        matrix2(tensor2_moved.data(), cols2, rows2);

    // Perform matrix multiplication
    // Eigen::Matrix<ResultType, Eigen::Dynamic, Eigen::Dynamic> result_matrix =
    // matrix1 * matrix2;
    Eigen::Matrix<ResultType, Eigen::Dynamic, Eigen::Dynamic> result_matrix =
        _gemm(matrix1, matrix2);
    ...
    std::copy(result_matrix.data(), result_matrix.data() + result_matrix.size(),
              result_tensor.data());
